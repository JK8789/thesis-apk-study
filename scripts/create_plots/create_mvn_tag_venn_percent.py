#!/usr/bin/env python3
"""
Equal-size Venn-style plot(s) for Maven SDK tags (RU vs EU) using PERCENTAGES.
...
Output directory:
  Plots/mvn_venn_percent/

For each tag (row), saves:
  <tag>_venn_percent.png
  <tag>_venn_percent.svg

Run from thesis-apk-study/:
  python3 scripts/create_plots/create_mvn_tag_venn_percent.py
"""
...
EXPLANATION = (
    "Percentages denote the share of unique Maven SDK libraries that are RU-only, EU-only, or shared for this tag "
    "(computed over RU-only + EU-only + shared)."
)
...
def fmt_pct(part: int, total: int) -> str:
    ...
    # nice formatting: no decimals if integer, else 1 decimal
    if abs(pct - round(pct)) < 1e-9:
        return f"{int(round(pct))}%"
    return f"{pct:.1f}%"


def draw_equal_venn_percent(ax, ru_only: int, eu_only: int, common: int) -> None:
    total = ru_only + eu_only + common

    # Fixed geometry (equal circles)
    r = 1.2
    dx = 1.0  # overlap amount
    ...
    # Percent labels
    ax.text(left_center[0] - 0.55, 0.0, fmt_pct(ru_only, total),
            ha="center", va="center", fontsize=26, color="black")
    ax.text(0.0, 0.0, fmt_pct(common, total),
            ha="center", va="center", fontsize=28, color="black", fontweight="bold")
    ax.text(right_center[0] + 0.55, 0.0, fmt_pct(eu_only, total),
            ha="center", va="center", fontsize=26, color="black")
...
def main() -> int:
    ...
    out_dir = Path("Plots/mvn_venn_percent")
    ...
    for i, row in df.iterrows():
        ...
        draw_equal_venn_percent(ax, ru_only=ru_only, eu_only=eu_only, common=common)

        base = slugify(tag)
        png_path = out_dir / f"{base}_venn_percent.png"
        svg_path = out_dir / f"{base}_venn_percent.svg"
