...
df['none_declared']=(df['collected_count'].fillna(0).astype(int).eq(0)&df['shared_count'].fillna(0).astype(int).eq(0))
summary=df.groupby('region',as_index=False).agg(none_declared=('none_declared','sum'), total_apps=('app_name','count'))
...
summary['none_declared']=summary['none_declared'].fillna(0).astype(int); summary['total_apps']=summary['total_apps'].fillna(20).astype(int)
summary['percent']=summary['none_declared']/summary['total_apps']*100
labels=['RU applications','EU applications']; values=summary['percent'].tolist(); counts=summary['none_declared'].tolist(); totals=summary['total_apps'].tolist()
...
fig.tight_layout(); out=OUT/'fig6_10_data_safety_none_bar.png'; fig.savefig(out,dpi=300,bbox_inches='tight'); print(out)
