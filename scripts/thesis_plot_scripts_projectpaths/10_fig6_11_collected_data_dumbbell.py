...
for i, row in enumerate(wide.itertuples(index=False)):
    ax.text(row.ru - 0.20, i + 0.07, f"{int(row.ru)}",
            ha="right", va="bottom", fontsize=9, color=ru_color)
    ax.text(row.eu + 0.20, i + 0.07, f"{int(row.eu)}",
            ha="left", va="bottom", fontsize=9, color=eu_color)

ax.set_yticks(y)
ax.set_yticklabels([PAIR_LABELS.get(p, p) for p in wide["pair_id"]], fontsize=10)
ax.set_xlabel("Number of declared collected data categories", fontsize=11)
