...
for _, row in pivot.iterrows():
    ...
    ax.text(-0.02, row["Google Play"], row["apk_filename"], ha="right", va="center", fontsize=8)
ax.set_xlim(-0.35, 1.15)
ax.set_xticks([0,1]); ax.set_xticklabels(["Google Play", "RuStore"])
ax.set_ylabel("Requested permissions count")
ax.set_title("Requested permissions comparison across stores", fontsize=16, pad=12)
ax.grid(axis="y", linestyle=":", alpha=0.35)
fig.tight_layout()
fig.savefig(OUT / "fig6_15_requested_permissions_slope.png", dpi=300, bbox_inches="tight")
print(OUT / "fig6_15_requested_permissions_slope.png")
