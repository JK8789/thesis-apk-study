...
OUTPUT_FILE = os.path.join(BASE_DIR, "results/libs_longest/androlibzoo_summary_faster.csv")
...
# ----------------------------
def get_parent_prefixes(prefix):
    parts = prefix.split(".")
    parents = []
    ...
    for fname in files:
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                # generate parent prefixes and match
                for candidate in get_parent_prefixes(prefix):
                    if candidate in androlibs:
                        lib_to_apps[candidate].add(sha)
...
def main():
    ...
    print("[3] Matching libraries (fast)...")
